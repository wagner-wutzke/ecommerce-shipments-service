...
```
```
docker build -t shipments-service:local .
```
```
docker network create ecommerce-net
```
```
docker run --rm --name shipments-service \
  --network ecommerce-net \
  -e KAFKA_BOOTSTRAP_SERVERS=kafka:9092 \
  -p 9050:9050 \
  shipments-service:local
```
Please be aware of the port being used in the docker command (9050) and the one configured in the
